...
def parseCommandLineArguments():
	...
	labelsPath = os.path.sep.join([args["Data"], "anive.names"])
	LABELS = open(labelsPath).read().strip().split("\n")
	
	# derive the paths to the YOLO weights and model configuration
	weightsPath = os.path.sep.join([args["Data"], "yolov5.weights"])
	configPath = os.path.sep.join([args["Data"], "yolov5.cfg"])
